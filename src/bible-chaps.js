...
const makeTrie = books => {
  const trie = trieNode({ data: null })

  Object.keys(books).forEach(book => {
    let temp = trie
    const b = book.toLowerCase()
    for (let i = 0; i < b.length; i++) {
      if (!temp.childs.hasOwnProperty(b[i])) {
        temp.childs[b[i]] = trieNode({ data: null })
      }
      temp = temp.childs[b[i]]
    }
    temp.word = book
    temp.data = books[book]
    temp.isEnd = true
  })
...
const search = (trie, text) => {
  ...
  for (let i = 0; i < lower.length; i++) {
    if (!trie.childs.hasOwnProperty(lower[i])) {
      return null
    }
    trie = trie.childs[lower[i]]
  }
  return trie != null && trie.isEnd ? trie.data : null
}

const getWords = trie => {
  if (trie == null || trie.childs == null) {
    return []
  }

  const words = []
  const nodes = Object.keys(trie.childs).map(k => trie.childs[k])
  ...
  while (current != null) {
    ...
    Object.keys(current.childs).forEach(c => nodes.push(current.childs[c]))
...
const fuzzySearch = (trie, text) => {
  if (text == null || text.length < 1) {
    return []
  }

  for (let i = 0; i < text.length; i++) {
    if (!trie.childs.hasOwnProperty(text[i])) {
      return []
    }
    trie = trie.childs[text[i]]
  }
